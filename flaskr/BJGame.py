...
#this section is for the player
    ...
    def deal1(self,hand):
        ...
        if len(self.deck) > 0:
            hand.insert(0,self.deck.pop())
        else:
            self.status = self.endGame

    def playerChoice(self):
        'Asks user to choose to stand or take a hit and takes action. Detects end of game.'
        while self.player.evalhd() < 21 :
            self.displayHands()
            choice = self.getInput('Enter "h" for hit or return for stand: ')
            if choice == "h":
                self.deal1(self.player)
                strings= []
                for card in self.player:
                ...
                self.show('** Bad input, use "h" or return')
        return

    def dealerStrategy(self):
        'Adds cards if dealer hand is less than 18'
        while self.dealer.evalhd() < 21:
            if self.dealer.evalhd() <= 17:
                self.deal1(self.dealer)
                ...
                if self.status == self.endGame:
                    return
                self.show('Dealer draws ' + str(self.dealer[0].rank))
            else:
                return

    def whoWon(self):
        'Uses higher hand to determine winner.'
        valued = self.dealer.evalhd()
        valuep = self.player.evalhd()
        if valued > 21:
    ...
    def displayHands(self):
        ...
        self.show("Player")
        for card in self.player:
            self.show(str(ranks[card.rank-1]) + ' ' + str(suits[card.suit]))
        self.show('Hand value ' + str(self.player.evalhd()))
    ...
    def displayDealerHand(self):
        ...
        for card in self.dealer:
            self.show(str(ranks[card.rank-1]) + ' ' + str(suits[card.suit]))
        self.show('Hand value ' + str(self.dealer.evalhd()))
    ...
    def getInput(self,strg):
        'Uses srandard input unless testing. Random selection in testing.'
        if not(self.test):
            return(input(strg))
        else:
            choice = ['h','','k','h','','h','','h','']
